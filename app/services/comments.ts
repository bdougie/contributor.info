...
function formatUsername(username: string, config?: ContributorConfig): string {
  ...
  return allowMentions
    ? `@${sanitizedUsername}`
    : `[${sanitizedUsername}](https://github.com/${sanitizedUsername})`;
}
...
export function formatMinimalPRComment(data: CommentData): string {
  ...
  if (reviewerSuggestions.length > 0) {
    comment += ` • Suggested reviewers: ${reviewerSuggestions
      .slice(0, 2)
      .map((r) => formatUsername(r.login, config))
      .join(', ')}`;
  }
...
export function formatErrorComment(repository?: Repository): string {
  const repoUrl = repository
    ? `https://contributor.info/${repository.full_name}`
  ...
  if (mayFix.length > 0) {
    ...
    mayFix.forEach((item) => {
      const matchPercent = Math.round(
        (item.similarity_score * 0.6 + item.file_overlap_score * 0.4) * 100
      );
