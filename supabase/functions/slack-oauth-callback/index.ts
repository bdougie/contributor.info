...
serve(async (req) => {
  ...
  try {
    ...
    // OAuth callbacks don't have authenticated user context
    // Always set created_by to NULL since we can't verify the user exists in auth.users
    // (workspace_members may have users that don't exist in auth schema)
    const userId = null;
