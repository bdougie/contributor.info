...
async function processPullRequestData(supabase: any, pr: any, repositoryId: string) {
  try {
    ...
    const detailedResponse = await fetch(
      `https://api.github.com/repos/${pr.base.repo.owner.login}/${pr.base.repo.name}/pulls/${pr.number}`,
      {
        headers: {
...
async function processEvents(supabase: any, events: GitHubEvent[], owner: string, repo: string) {
  ...
  for (const event of events) {
    ...
    // Store event
    await supabase.from('github_events_cache').insert({
      event_id: eventId,
  ...
  try {
    ...
    for (const repo of repositories) {
      try {
        ...
        // Update sync status
        const lastEventAt = events.length > 0 ? events[0].created_at : syncStatus?.last_event_at;
...
async function invalidateRepositoryConfidenceCache(supabase: any, owner: string, repo: string) {
  try {
