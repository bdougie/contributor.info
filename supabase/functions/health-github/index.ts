...
serve(async (req) => {
  ...
  try {
    ...
    const httpStatus = overallHealthy ? 200 : basicTest.status === 'unhealthy' ? 503 : 200;
...
function analyzeRateLimits(rateLimits: any): {
  ...
  const status = critical.length > 0 ? 'critical' : warnings.length > 0 ? 'warning' : 'healthy';
...
function generateRecommendations(basicTest: any, authTest: any, rateLimitHealth: any): string[] {
