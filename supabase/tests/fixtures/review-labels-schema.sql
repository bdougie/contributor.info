...
CREATE TABLE public.workspace_members(id uuid DEFAULT gen_random_uuid(),workspace_id uuid REFERENCES workspaces(id),user_id uuid REFERENCES app_users(id),role text CHECK(role IN ('owner','maintainer','contributor')),invited_by uuid REFERENCES app_users(id),accepted_at timestamptz,UNIQUE(workspace_id,user_id));
