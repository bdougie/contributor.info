...
test.describe('Core Web Vitals', () => {
  ...
  test('Interaction responsiveness (INP proxy)', async ({ page }) => {
    ...
    expect(interactionDelay, 'Interaction delay should be under 200ms').toBeLessThan(PERFORMANCE_THRESHOLDS.INP);
  });
});
...
test.describe('Performance Budget', () => {
  test('JavaScript bundle size is within budget', async ({ page }) => {
    ...
    expect(totalJsSize, `Total JS size (${totalJsSize} bytes) should be under 350KB`).toBeLessThan(maxJsSize);
  });
