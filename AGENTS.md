...
- **Function grants** — default privileges in `public` no longer grant EXECUTE to PUBLIC (revoked in migration `20260707000002`). Any new function meant to be called from the frontend via `supabase.rpc()` needs an explicit `GRANT EXECUTE ON FUNCTION public.my_rpc(args) TO authenticated;` (add `anon` only if it must work logged-out). Without the grant, frontend calls fail with 42501 permission denied.
