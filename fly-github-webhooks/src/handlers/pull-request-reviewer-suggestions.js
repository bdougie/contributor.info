...
export async function handlePRWithReviewerSuggestions(payload, githubApp, supabase, parentLogger) {
  const logger = parentLogger
    ? parentLogger.child('PRReviewerSuggestions')
    : new Logger('PRReviewerSuggestions');
  ...
  logger.info(
    'Processing PR #%s (%s) in %s for reviewer suggestions',
    pr.number,
    action,
    repo.full_name
  );
...
async function findFileContributors(filePaths, repo, supabase, logger) {
  try {
    ...
    if (repoError || !repoData) {
      logger.error(
        'Repository not found for file contributors:',
        repoError?.message || 'Not tracked'
      );
      return [];
    }
