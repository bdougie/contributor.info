/**
 * SVG generation for social cards, rendered server-side in a Netlify Function.
 * Ported from fly-social-cards/src/card-generator.js so cards keep the exact
 * same design; text uses the vendored Inter family (render.ts) instead of
 * system-ui, which does not exist in Lambda.
 *
 * Missing stats render as zeros — never as mock figures — so a card for a
 * repo we have no data on cannot present fabricated numbers as real.
 */
...
const faviconLogo = (x: number, y: number, size = 28): string => `
...
// Icons as SVG paths - scaled to 32x32; Lucide icons use a 24x24 viewBox
const ICON_SCALE = 1.33;
const icons = {
  users: (x: number, y: number, color: string) => `
  ...
  `,
  gitPullRequest: (x: number, y: number, color: string) => `
    <g transform="translate(${x}, ${y}) scale(${ICON_SCALE})">
  ...
  `,
  trendingUp: (x: number, y: number, color: string) => `
    <g transform="translate(${x}, ${y}) scale(${ICON_SCALE})">
...
const header = (): string => `
    <g transform="translate(48, 40)">
      ${faviconLogo(0, 0, 28)}
      <text x="36" y="20" font-size="22" font-weight="600" fill="${THEME.text}" font-family="${FONT}">contributor.info</text>
    </g>
`;

const generateHomeCard = (stats: GlobalCardStats | null): string => {
  const contributorsNum = `${formatNumber(stats?.contributors ?? 0)}+`;
  const pullRequestsNum = `${formatNumber(stats?.pullRequests ?? 0)}+`;
  const repositoriesNum = `${formatNumber(stats?.repositories ?? 0)}+`;

  return `<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
    <rect width="1200" height="630" fill="${THEME.background}"/>
    ${header()}

    <text x="48" y="260" font-size="56" font-weight="700" fill="${THEME.text}" font-family="${FONT}">Open Source Insights</text>
    <text x="48" y="320" font-size="24" fill="${THEME.textMuted}" font-family="${FONT}">Visualizing contributions across the ecosystem</text>

    <g transform="translate(48, 440)">
      <g transform="translate(0, 0)">
        ${icons.users(0, -8, THEME.primary)}
        <text x="44" y="0" font-size="32" font-weight="700" fill="${THEME.text}" font-family="${FONT}">${contributorsNum}</text>
        <text x="44" y="28" font-size="18" fill="${THEME.textMuted}" font-family="${FONT}">Contributors</text>
      </g>
      <g transform="translate(280, 0)">
        ${icons.gitPullRequest(0, -8, THEME.primary)}
        <text x="44" y="0" font-size="32" font-weight="700" fill="${THEME.text}" font-family="${FONT}">${pullRequestsNum}</text>
        <text x="44" y="28" font-size="18" fill="${THEME.textMuted}" font-family="${FONT}">Pull Requests</text>
      </g>
      <g transform="translate(560, 0)">
        ${icons.trendingUp(0, -8, THEME.primary)}
        <text x="44" y="0" font-size="32" font-weight="700" fill="${THEME.text}" font-family="${FONT}">${repositoriesNum}</text>
        <text x="44" y="28" font-size="18" fill="${THEME.textMuted}" font-family="${FONT}">Repositories</text>
      </g>
    </g>
  </svg>`;
};
...
const generateRepoCard = (
  ...
  const remainingContributors = Math.max((stats?.totalContributors ?? 0) - 5, 0);

  return `<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
    <rect width="1200" height="630" fill="${THEME.background}"/>
    ${header()}

    <text x="48" y="260" font-size="56" font-weight="700" fill="${THEME.text}" font-family="${FONT}">${safeTitle}</text>
    <text x="48" y="320" font-size="24" fill="${THEME.textMuted}" font-family="${FONT}">Past 6 months</text>

    <g transform="translate(48, 440)">
      <g transform="translate(0, 0)">
        ${icons.trendingUp(0, -8, THEME.primary)}
        <text x="44" y="0" font-size="32" font-weight="700" fill="${THEME.primary}" font-family="${FONT}">${weeklyPRNum}</text>
        <text x="44" y="28" font-size="18" fill="${THEME.textMuted}" font-family="${FONT}">Weekly PR Volume</text>
      </g>
      <g transform="translate(320, 0)">
        ${icons.users(0, -8, THEME.text)}
        <text x="44" y="0" font-size="32" font-weight="700" fill="${THEME.text}" font-family="${FONT}">${activeContribNum}</text>
        <text x="44" y="28" font-size="18" fill="${THEME.textMuted}" font-family="${FONT}">Active Contributors</text>
      </g>
    </g>
...
const generateUserCard = (title: string): string => {
  const safeTitle = escapeHtml(title);
  ...
  return `<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="48" y="280" font-size="56" font-weight="700" fill="${THEME.text}" font-family="${FONT}">${safeTitle}</text>
    <text x="48" y="340" font-size="24" fill="${THEME.textMuted}" font-family="${FONT}">Open Source Contributor</text>
  </svg>`;
};

const generateErrorCard = (
  title: string,
  subtitle: string
): string => `<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
    <rect width="1200" height="630" fill="${THEME.background}"/>
    <text x="600" y="300" text-anchor="middle" font-size="48" font-weight="600" fill="#EF4444" font-family="${FONT}">${escapeHtml(title)}</text>
    <text x="600" y="360" text-anchor="middle" font-size="24" fill="${THEME.textMuted}" font-family="${FONT}">${escapeHtml(subtitle)}</text>
  </svg>`;
...
export function generateSocialCard(data: SocialCardData): string {
  switch (data.type) {
    ...
    case 'error':
      return generateErrorCard(data.title, data.subtitle);
    default:
      return generateHomeCard(data.stats);
  }
}
