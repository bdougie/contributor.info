    ...
    const syncResults = await Promise.allSettled(
      repositoryIds.map(async (repoId: string) => {
        try {
          ...
          console.log(`[workspace-sync] Marked repository %s for sync`, `${data.owner}/${data.name}`);
          ...
          console.error(`[workspace-sync] Failed to mark repository %s for sync:`, repoId, error);
          return {
            repositoryId: repoId,
            status: 'failed',
