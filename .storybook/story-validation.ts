...
export function validateStory(
  storyExports: any,
  config: StoryValidationConfig = defaultValidationConfig
): {
  isValid: boolean;
  errors: string[];
  warnings: string[];
  ...
  if (!meta.component) {
    warnings.push(
      'Meta configuration should include a component reference for better prop detection'
    );
  }
  ...
  if (config.requireDocumentation) {
    if (!meta.parameters?.docs?.description?.component) {
      warnings.push(
        'Component should include documentation in meta.parameters.docs.description.component'
      );
    }
  }
  ...
  const missingRecommended = config.recommendedStories.filter((story) => !stories.includes(story));
...
export function validateStoryFile(
  ...
  return {
    ...
    summary: {
      totalErrors:
        structureValidation.errors.length +
        accessibilityValidation.errors.length +
        performanceValidation.errors.length,
      totalWarnings:
...
export function scoreStoryQuality(storyExports: any): {
  ...
  const score = Math.round(
    (structureScore + documentationScore + coverageScore + accessibilityScore + performanceScore) /
      5
  );
