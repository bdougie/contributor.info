...
const supabaseAnonKey =
  ...
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVnY3h6b25wbW1jaXJtZ3FkcmxhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTIxODAzNzEsImV4cCI6MjA2Nzc1NjM3MX0.SY1LMsRFyrBtHiZfgDhXD9ZlKl37-L7Uar4HnyDgw24';
...
const staticPages = [
  ...
  {
    loc: '/privacy/data-request',
    priority: PRIORITY_LEVELS.lowPriority,
...
async function generateSitemap() {
  ...
  for (const doc of docPages) {
    xml += generateUrl(`/docs/${doc}`, PRIORITY_LEVELS.docs, CHANGE_FREQ.docs);
  }
  ...
  for (const repo of repositories) {
    ...
    xml += generateUrl(repoPath, priority, CHANGE_FREQ.repository, lastmod);

    // Repository subpages (only for higher priority repos)
    if (priority >= 0.75) {
      xml += generateUrl(`${repoPath}/health`, priority - 0.05, CHANGE_FREQ.repository, lastmod);
