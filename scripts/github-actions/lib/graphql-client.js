...
class GraphQLClient {
  ...
  async query(query, variables = {}) {
    ...
    if (data.errors) {
      ...
      if (rateLimitError) {
        ...
        const resetAt = data.data?.rateLimit?.resetAt || new Date(Date.now() + 3600000).toISOString();
  ...
  async checkRateLimitBeforeQuery(estimatedCost = 1, minRemainingPoints = 100) {
    ...
    if (rateLimit.remaining < minRemainingPoints) {
      const error = new Error(`GraphQL rate limit too low: ${rateLimit.remaining} points remaining (minimum: ${minRemainingPoints})`);
    ...
    if (rateLimit.remaining < estimatedCost) {
      const error = new Error(`Insufficient rate limit for query. Need ${estimatedCost} points, have ${rateLimit.remaining}`);
