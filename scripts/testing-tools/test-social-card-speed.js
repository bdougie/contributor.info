...
const testUrls = [
  ...
  {
    name: 'Custom Title',
    url: `${baseUrl}/social-cards/home?title=Custom%20Title&subtitle=Custom%20Subtitle`,
  },
];
...
async function runSpeedTest() {
  ...
  results.forEach((result, index) => {
    ...
    console.log(
      `   Response Time: ${result.avgTime}ms (min: ${result.minTime}ms, max: ${result.maxTime}ms)`
    );
    ...
    if (result.headers) {
      ...
      console.log(
        `   Server Response Time: ${result.headers.responseTimeHeader || 'Not reported'}`
      );
    }
    console.log('');
  });
  ...
  if (typeof performance !== 'undefined' && performance.getEntriesByType) {
    ...
    try {
      ...
      if (entries.length > 0) {
        ...
        console.log(
          `- SSL Handshake: ${(entry.secureConnectionStart > 0 ? entry.connectEnd - entry.secureConnectionStart : 0).toFixed(2)}ms`
        );
        console.log(
          `- Request/Response: ${(entry.responseStart - entry.requestStart).toFixed(2)}ms`
        );
        console.log(
          `- Response Download: ${(entry.responseEnd - entry.responseStart).toFixed(2)}ms`
        );
