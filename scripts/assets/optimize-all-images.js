...
async function optimizeImage(imagePath, metadata) {
  ...
  try {
    ...
    await sharp(imagePath).webp({ quality: QUALITY_SETTINGS.webp }).toFile(webpPath);
    ...
    if (metadata.format === 'png' && metadata.size > SIZE_THRESHOLDS.small) {
      ...
      await sharp(imagePath).png(QUALITY_SETTINGS.png).toFile(optimizedPngPath);
      ...
      if (optimizedStats.size < metadata.size * 0.9) {
        ...
        results.savings.png = (
          ((metadata.size - optimizedStats.size) / metadata.size) *
          100
        ).toFixed(1);
      } else {
