...
function readEnvFile(filePath) {
  try {
    ...
    if (stats.size > MAX_ENV_FILE_SIZE) {
      throw new Error(
        `Security Error: File too large (${stats.size} bytes). Maximum allowed: ${MAX_ENV_FILE_SIZE} bytes`
      );
    }
    ...
    content.split('\n').forEach((line, index) => {
      ...
      if (match) {
        ...
        if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
          console.warn(
            `Warning: Skipping invalid environment variable key at line ${index + 1}: ${key}`
          );
          return;
        }
...
async function switchEnvironment(envName) {
  ...
  try {
    ...
    const envExamplePath = validateFilePath(
      path.join(SAFE_ROOT_DIR, '.env.example'),
      SAFE_ROOT_DIR
    );
    ...
    if (deletedCount > 0) {
      log(
        `\nCleaned up ${deletedCount} old backup file(s) (kept last ${MAX_BACKUP_FILES})`,
        colors.cyan
      );
    }
