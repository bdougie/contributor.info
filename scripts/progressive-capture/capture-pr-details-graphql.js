...
class PRDetailsGraphQLCaptureScript extends BaseCaptureScript {
  ...
  async processItem(pr) {
    ...
    try {
      ...
      const { error: prError } = await this.supabase.from('pull_requests').upsert(prRecord, {
      ...
      this.log(
        `✓ Captured complete data for PR #${pr.number} (${files.length} files, ${reviews.length} reviews, ${allComments.length} comments)`
      );
    } catch (error) {
      if (error.status === 404) {
