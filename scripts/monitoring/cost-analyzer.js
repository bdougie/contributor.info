...
class CostAnalyzer {
  ...
  async analyzeCosts() {
    ...
    try {
      ...
      await this.generateCostReport(historicalCosts, currentCosts, savings, projections, recommendations);
  ...
  calculateCosts(jobs, period) {
    ...
    if (inngestJobs.length > 0) {
      ...
      const totalItems = inngestJobs.reduce((sum, job) => 
        sum + (job.metadata?.processed_items || job.metadata?.max_items || 10), 0
      );
    ...
    if (actionsJobs.length > 0) {
      ...
      const totalItems = actionsJobs.reduce((sum, job) => 
        sum + (job.metadata?.processed_items || job.metadata?.max_items || 100), 0
      );
      costs.github_actions.avgCostPerItem = totalItems > 0 ? costs.github_actions.cost / totalItems : 0;
    }

    // Calculate hybrid overhead
    if (period === 'current') {
      costs.hybrid.overhead = jobs.length * this.costModels.hybrid.routingOverhead +
  ...
  calculateGitHubActionsCosts(jobs) {
    ...
    for (const job of jobs) {
      ...
      const actualDuration = Math.max(durationMinutes, this.costModels.github_actions.setupOverhead);
  ...
  calculateSavings(historical, current) {
    ...
    return {
      ...
      percentage: {
        ...
        inngest: ((normalizedHistorical.inngest.cost - current.inngest.cost) / normalizedHistorical.inngest.cost) * 100,
        efficiency: (current.github_actions.avgCostPerItem < current.inngest.avgCostPerItem) ? 
                   ((current.inngest.avgCostPerItem - current.github_actions.avgCostPerItem) / current.inngest.avgCostPerItem) * 100 : 0
      },
      breakdown: {
  ...
  generateCostOptimizations(currentCosts, savings) {
    ...
    if (currentCosts.total > 50) { // If monthly costs > $50
      recommendations.push({
  ...
  displayCostAnalysis(costs) {
    ...
    console.log(`  ⚡ Inngest: ${costs.inngest.jobs} jobs, $${costs.inngest.cost.toFixed(4)} ($${costs.inngest.avgCostPerItem.toFixed(6)}/item)`);
    console.log(`  🔧 GitHub Actions: ${costs.github_actions.jobs} jobs, $${costs.github_actions.cost.toFixed(4)} ($${costs.github_actions.avgCostPerItem.toFixed(6)}/item)`);
  ...
  async generateCostReport(historical, current, savings, projections, recommendations) {
    ...
    console.log(`💯 Target achieved: ${report.targets.achieved ? 'YES' : 'NO'} (${report.targets.actualSavings.toFixed(1)}% vs ${report.targets.targetRange})`);
