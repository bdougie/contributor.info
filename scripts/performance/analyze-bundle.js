  ...
  try {
    ...
    if (byType['.js']) {
      ...
      byType['.js'].forEach((file, index) => {
        ...
        console.log(
          `${index + 1}. ${file.name} - ${formatBytes(file.size)} (${formatBytes(gzipEst)} gzipped)`
        );
    ...
    console.log(`\n📈 Bundle Composition:`);
    console.log(
      `   JavaScript: ${formatBytes(totalJS)} (${Math.round((totalJS / totalSize) * 100)}%)`
    );
    ...
    console.log(`\n⚡ Performance Impact:`);
    console.log(
      `   LCP Impact: ${criticalGzipped < 150000 ? 'Low' : criticalGzipped < 200000 ? 'Medium' : 'High'}`
    );
