...
async function analyzeLighthouseReport(reportPath) {
  try {
    ...
    const bundles = {
      totalJS: resourceSummary?.details?.items?.find(item => item.resourceType === 'script')?.size || 0,
      totalCSS: resourceSummary?.details?.items?.find(item => item.resourceType === 'stylesheet')?.size || 0,
      totalImages: resourceSummary?.details?.items?.find(item => item.resourceType === 'image')?.size || 0
    };
...
async function generateMobilePerformanceReport() {
  ...
  try {
    ...
    if (analyses.length === 0) {
      console.log('❌ No mobile lighthouse reports found. Run npm run test:lighthouse:mobile first.');
      return;
    }
    ...
    const avgScore = analyses.reduce((sum, a) => sum + a.metrics.performanceScore, 0) / analyses.length;
    ...
    analyses.forEach(analysis => {
      const rating = analysis.metrics.performanceScore >= 90 ? '🟢' : 
                    analysis.metrics.performanceScore >= 75 ? '🟡' : '🔴';
      console.log(`${rating} ${analysis.report}: ${analysis.metrics.performanceScore.toFixed(1)}/100`);
    });
