...
#### Before (scripts/embeddings/trigger-embeddings.mjs:10-27)
```javascript
...
#### After (scripts/embeddings/trigger-embeddings.mjs:8-31)
```javascript
...
1. `scripts/embeddings/trigger-embeddings.mjs` - Fixed to use Inngest API
2. `scripts/embeddings/backfill-embeddings-priority.sh` - Updated env var validation
...
./scripts/embeddings/backfill-embeddings-priority.sh 20 5
```
