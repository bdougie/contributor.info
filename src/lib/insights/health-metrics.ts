...
import type { PullRequest, RepoStats } from '../types';
...
export function calculateHealthMetricsFromStats(
  stats: RepoStats,
  timeRange: string
): HealthMetrics {
  const now = new Date();
  ...
  const activityScore = Math.round(Math.min(100, dailyActivity * 20));
