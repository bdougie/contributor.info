...
export async function calculateHealthMetrics(
  ...
  try {
    ...
    const uniqueContributors = new Set(
      pullRequests.map((pr: unknown) => pr.user?.login).filter(Boolean),
    );
    ...
    const prsWithReviews = pullRequests.filter(
      (pr: unknown) => pr.reviews && pr.reviews.length > 0,
    ).length;
  ...
  try {
    ...
    if (!forceRecalculate) {
      ...
      if (cachedResult !== null) {
        console.log(
          '[Confidence] Using _database cached score for %s/%s: %s%',
          owner,
          repo,
          cachedResult.score,
        );
...
async function calculateStarForkConfidence(
  ...
  const stargazers = new Set(
    starForkEvents
      .filter((e: unknown) => e.event_type === 'WatchEvent')
      .map((e: unknown) => e.actor_login),
  );
  const forkers = new Set(
    starForkEvents
      .filter((e: unknown) => e.event_type === 'ForkEvent')
      .map((e: unknown) => e.actor_login),
  );
...
async function calculateEngagementConfidence(
  ...
  const engagers = new Set(
    engagementEvents?.map((e: unknown) => e.actor_login).filter(Boolean) || [],
  );
