...
function createChainableMock(finalResult: unknown) {
  ...
  // This ensures it resolves correctly when awaited directly (for queries without .maybeSingle())
...
describe('calculateRepositoryConfidence', () => {
  ...
  describe('Fallback Calculation', () => {
    it('should use fallback calculation when event _data is unavailable', async () => {
      const { supabase } = await import('@/lib/supabase');
