    ...
    try {
      ...
      for (const review of reviews) {
        try {
          ...
          if (reviewerError) {
            console.warn(
              `[Reviews Processor] Error upserting reviewer ${review.user.login}:`,
              reviewerError
            );
            continue;
          }
          ...
          // Insert review
          const { error: reviewInsertError } = await supabase.from('reviews').upsert(
            {
              github_id: review.id,
  ...
  static async processCommentsJob(
    ...
    try {
      ...
      const [reviewCommentsResponse, issueCommentsResponse] = await Promise.all([
        fetch(
          `${this.GITHUB_API_BASE}/repos/${repo.owner}/${repo.name}/pulls/${prNumber}/comments`,
          { headers }
        ),
        fetch(
          `${this.GITHUB_API_BASE}/repos/${repo.owner}/${repo.name}/issues/${prNumber}/comments`,
          { headers }
        ),
      ]);
      ...
      if (reviewCommentsResponse.ok) {
        ...
        console.warn(
          `[Comments Processor] Error fetching review comments: ${reviewCommentsResponse.status}`
        );
      }
      ...
      if (issueCommentsResponse.ok) {
        ...
        console.warn(
          `[Comments Processor] Error fetching issue comments: ${issueCommentsResponse.status}`
        );
      }
      ...
      for (const comment of allComments) {
        try {
          ...
          if (commenterError) {
            console.warn(
              `[Comments Processor] Error upserting commenter ${comment.user.login}:`,
              commenterError
            );
            continue;
          }
          ...
          // Insert comment
          const { error: commentInsertError } = await supabase.from('comments').upsert(
            {
          ...
          console.warn(
            `[Comments Processor] Error processing comment ${comment.id}:`,
            commentError
          );
        }
      }
