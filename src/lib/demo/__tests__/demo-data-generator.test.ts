...
describe('Demo Data Generator', () => {
  ...
  describe('generateDemoWorkspaceTrendData', () => {
    ...
    it('should scale data based on number of repositories', () => {
      ...
      const avgAdditionsWithRepos = dataWithRepos.reduce((sum, d) => sum + d.additions, 0) / dataWithRepos.length;
      const avgAdditionsWithoutRepos = dataWithoutRepos.reduce((sum, d) => sum + d.additions, 0) / dataWithoutRepos.length;
