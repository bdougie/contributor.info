...
export const captureRepositorySyncGraphQL = inngest.createFunction(
  ...
  async ({ event, step }) => {
      ...
      if (_data.last_updated_at && !hasActiveBackfill) {
        ...
        if (hoursSinceSync < effectiveThrottleHours) {
          ...
          if (!hasCompleteData && hoursSinceSync < 0.083) {
            ...
            throw new Error(
              `Repository ${data.owner}/${data.name} was synced ${timeAgo} ago and ${_dataStatus}. Skipping to prevent excessive API usage.`,
    ...
    const storedPRs = await step.run('store-prs', async () => {
      ...
      const contributorPromises = recentPRs.map((pr: unknown) =>
        ensureContributorExists(pr.author),
      );
