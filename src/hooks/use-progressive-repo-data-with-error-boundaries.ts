...
export function useProgressiveRepoDataWithErrorBoundaries(
  ...
  const handleStageError = useCallback(
    async (_error: unknown, stage: LoadingStage, context?: unknown) => {
      const loadingError = error instanceof Error && (_error as LoadingError).stage
  ...
  const loadCriticalData = useCallback(async (owner: string, repo: string) => {
    return startSpan({ name: 'progressive-load-critical-enhanced' }, async (span) => {
      try {
        ...
        const canContinue = await handleStageError(__error, 'critical', { owner, repo, timeRange });
        if (!canContinue) {
          throw error;
        }
        return null;
  ...
  const loadFullData = useCallback(async (owner: string, repo: string) => {
    return startSpan({ name: 'progressive-load-full-enhanced' }, async (span) => {
      try {
        ...
        if (!result._data && !enableGracefulDegradation) {
...
function createGenericLoadingError(
  ...
  let configKey: keyof typeof import('@/lib/types/_data-loading-_errors').ERROR_CONFIGS = 'NETWORK_TIMEOUT';
