...
describe('useGitHubApi', () => {
  ...
  it('should fetch repository information', async () => {
    ...
    expect(fetchRepositoryMock).toHaveBeenCalledWith('testuser', 'test-repo', {
  ...
  it('should fetch user organizations', async () => {
    ...
    expect(result.current.isLoading).toBe(false);
    expect(fetchUserOrganizationsMock).toHaveBeenCalledWith('testuser', {
      token: 'mock-github-token',
    });
  });
  ...
  it('should fetch pull requests', async () => {
    ...
    expect(result.current.isLoading).toBe(false);
    expect(fetchPullRequestsMock).toHaveBeenCalledWith('testuser', 'test-repo', 'all', {
      token: 'mock-github-token',
    });
  });
