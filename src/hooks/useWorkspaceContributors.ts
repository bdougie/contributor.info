...
export function useWorkspaceContributors({
  ...
  const fetchWorkspaceContributors = useCallback(
    async (availableContributors: Contributor[]) => {
      try {
  ...
  const refetchContributors = useCallback(async () => {
    setLoading(true);
    setError(null);
  ...
  }, [repositories, selectedRepositories, fetchWorkspaceContributors]);
  ...
  }, [repositories.length, refetchContributors]);
