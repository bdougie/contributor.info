...
export function CommandPalette({
  ...
  const { query, filter } = useMemo(() => {
    ...
    if (
      trimmedSearch.toLowerCase().startsWith('workspace:') ||
      trimmedSearch.toLowerCase().startsWith('w:')
    ) {
    ...
    if (
      trimmedSearch.toLowerCase().startsWith('repo:') ||
      trimmedSearch.toLowerCase().startsWith('r:')
    ) {
  ...
  const commands = useMemo<CommandItem[]>(() => {
    ...
    repositories.forEach((repo) => {
      items.push({
        ...
        description:
          repo.description ||
          `${repo.language || 'Unknown'} • ${formatStars(repo.stars || 0)} stars`,
    ...
    <CommandDialog open={open} onOpenChange={onOpenChange} aria-label="Command palette">
      ...
      <CommandList aria-label="Search results">
          ...
          <>
            <CommandGroup heading="Recent">
              {groupedCommands.recent.map((cmd) => (
                <CommandItem key={cmd.id} onSelect={cmd.action} className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {cmd.metadata?.tier && (
                      <Badge
                        variant="secondary"
                        className={cn('text-xs', getTierColor(cmd.meta_data.tier || ''))}
                      >
                        {cmd.metadata.tier}
                      </Badge>
                    )}
        ...
        {groupedCommands.action.length > 0 && (
          <>
            {(groupedCommands.workspace.length > 0 || groupedCommands.repository.length > 0) && (
              <CommandSeparator />
            )}
