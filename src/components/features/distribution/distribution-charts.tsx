...
function DistributionCharts({
  ...
  const [contributorView, setContributorView] = useState<'overview' | 'quadrant' | 'contributor'>(
    'overview',
  );
  ...
  const renderDonutChart = () => {
    return (
      ...
      >
        ...
        {tooltipData && (
          <div
            className="absolute pointer-events-none bg-background/95 backdrop-blur-sm border rounded-xl shadow-2xl p-4 max-w-xs z-50 transition-all duration-200 ease-out"
            style={{
      ...
      {data.map((item) => (
        <button
          ...
          className={`flex items-start gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg transition-all text-left ${
            activeSegment === item.id ? 'bg-accent ring-2 ring-primary' : 'hover:bg-accent/50'
          }`}
        >
          <div
  ...
  const renderPRDrawer = () => {
    ...
    return (
      <>
        ...
        <div
          className={`fixed sm:absolute top-0 right-0 h-full bg-background border-l shadow-lg transition-transform duration-300 ease-in-out z-40 ${
          ...
          <div className="p-4 h-full flex flex-col">
            <div className="flex items-center justify-between mb-4 flex-shrink-0">
              <div className="flex items-center gap-2 min-w-0">
                ...
                />
                <span className="font-semibold text-sm truncate">{selectedData?.label}</span>
              </div>
              <Button
                variant="ghost"
                size="sm"
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-2 mb-3">
            <div
              ...
              style={{ backgroundColor: COLORS[selectedQuadrant as keyof typeof COLORS] }}
            />
            <h4 className="font-semibold text-sm">{selectedData?.label}</h4>
          </div>
      ...
      {chartType === 'treemap' ? (
        <Card className="relative">
          <CardContent className="p-4 sm:p-6 overflow-visible">
            ...
            <div className="h-[350px] sm:h-[450px] overflow-visible">{renderTreemap()}</div>
