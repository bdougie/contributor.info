...
const isExtendedLotteryFactor = (factor: unknown): factor is ExtendedLotteryFactorType => {
  ...
  const obj = factor as Record<string, unknown>;
  return typeof obj.score === 'number' && typeof obj.rating === 'string';
};
...
export function RepoStatsSummary({ owner, repo }: RepoStatsSummaryProps) {
  ...
  const { stats, lotteryFactor, directCommitsData, getContributorStats, getFilteredPullRequests } =
    useRepoStats();
  ...
  // Handle cleanup of refresh timeout - moved before early returns
  useEffect(() => {
  ...
  // Get the most recent PR - memoized for performance - moved before early returns
  ...
  const handleManualRefresh = async () => {
    ...
    try {
      ...
      const sizeInfo = metadata?.size
        ? ` (${metadata.size.toUpperCase()} repo, ${refreshTimeRange} days)`
        : '';
    ...
    <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              {owner && repo && <RepositoryInlineMetadata owner={owner} repo={repo} />}
            </CardTitle>
            <CardDescription>Summary of repository activity and health metrics</CardDescription>
          </div>
          <Button
            variant="ghost"
            size="sm"
      ...
      <CardContent className="grid gap-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-medium text-muted-foreground">Lottery Factor</h3>
            <div className="text-2xl font-bold">
              {isExtendedLotteryFactor(lotteryFactor) ? lotteryFactor.score.toFixed(1) : 'N/A'}
            </div>
            <p className="text-xs text-muted-foreground">
              {isExtendedLotteryFactor(lotteryFactor) ? lotteryFactor.rating : 'Not calculated'}
            </p>
          </div>
        </div>
        ...
        {directCommitsData?.hasYoloCoders && (
          <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-md p-3">
            ...
            <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">
              ...
              {directCommitsData.yoloCoderStats.length !== 1 ? 's have' : ' has'} pushed commits
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{mostRecentPR.title}</div>
