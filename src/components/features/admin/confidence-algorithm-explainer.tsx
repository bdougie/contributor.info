...
export function ConfidenceAlgorithmExplainer({
  weights,
  onClose,
}: ConfidenceAlgorithmExplainerProps) {
  ...
  const calculateMockScore = () => {
    ...
    const privilegedRatio =
      inputs.totalEventCount > 0 ? inputs.privilegedEventCount / inputs.totalEventCount : 0;
    ...
    const getRecencyScore = (daysSinceLastSeen: number): number => {
    ...
    const recencyScore = getRecencyScore(inputs.daysSinceLastSeen);
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {(currentWeights.privileged_events_weight * 100).toFixed(0)}%
                  </Badge>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {(currentWeights.activity_patterns_weight * 100).toFixed(0)}%
                  </Badge>
                </div>
                <p className="text-sm text-muted-foreground">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {(currentWeights.temporal_consistency_weight * 100).toFixed(0)}%
                  </Badge>
                </div>
                <p className="text-sm text-muted-foreground">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700">
                    This algorithm measures the likelihood of outsider "self-selection" for
                    contributions. It deliberately <strong>excludes maintainers</strong> from
                    calculations and is capped at 50% to reflect realistic conversion rates.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput('privilegedEventCount', parseInt(e.target.value) || 0)
                    }
                    min="0"
                  />
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput('daysSinceLastSeen', parseInt(e.target.value) || 0)
                    }
                    min="0"
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput('daysSinceFirstSeen', parseInt(e.target.value) || 0)
                    }
                    min="0"
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput('activitySpreadDays', parseInt(e.target.value) || 0)
                    }
                    min="0"
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex items-center justify-between text-sm mb-1">
                      ...
                      <span className="font-medium">
                        {(score.components.privilegedEvents * 100).toFixed(1)}%
                      </span>
                    </div>
                    <Progress value={score.components.privilegedEvents * 100} className="h-2" />
                    <p className="text-xs text-muted-foreground mt-1">
                      Ratio: {(score.factors.privilegedRatio * 100).toFixed(1)}% + Boost:{' '}
                      {(score.factors.privilegedBoost * 100).toFixed(1)}%
                    </p>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between text-sm mb-1">
                      ...
                      <span className="font-medium">
                        {(score.components.activityPatterns * 100).toFixed(1)}%
                      </span>
                    </div>
                    ...
                    <div className="text-xs text-muted-foreground mt-1 space-y-0.5">
                      ...
                      <div>
                        Method Diversity: {(score.factors.methodDiversity * 100).toFixed(1)}%
                      </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between text-sm mb-1">
                      ...
                      <span className="font-medium">
                        {(score.components.temporalConsistency * 100).toFixed(1)}%
                      </span>
                    </div>
                ...
                <div className="text-sm">
                  ...
                  {score.overall * 100 <= 5 && (
                    <p className="text-red-600">
                      Critical - Very low likelihood of external contribution
                    </p>
                  )}
                  {score.overall * 100 > 5 && score.overall * 100 <= 15 && (
                    <p className="text-orange-600">
                      Low - Few external contributors are likely to contribute
                    </p>
                  )}
                  {score.overall * 100 > 15 && score.overall * 100 <= 35 && (
                    <p className="text-blue-600">
                      Medium - Some external contributors may contribute
                    </p>
                  )}
                  {score.overall * 100 > 35 && (
                    <p className="text-green-600">
                      Good - External contributors are more likely to contribute
                    </p>
                  )}
                </div>
              </div>
