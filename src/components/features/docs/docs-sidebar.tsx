...
export function DocsSidebar({ className }: DocsSidebarProps) {
  ...
  return (
    <aside className={cn('w-64 shrink-0 hidden lg:block', className)}>
      <nav
        className="sticky top-20 space-y-6 max-h-[calc(100vh-6rem)] overflow-y-auto pr-4 bg-transparent"
        ...
        <div>
          <Link
            ...
            className={cn(
              ...
              isDocsHome ? 'text-foreground' : 'text-muted-foreground hover:text-foreground',
            )}
          >
...
: (
            ...
            <span className="ml-auto text-xs text-muted-foreground">{featureDocs.length}</span>
          </button>
          {featuresExpanded && (
...
: (
            ...
            <span className="ml-auto text-xs text-muted-foreground">{insightDocs.length}</span>
          </button>
          {insightsExpanded && (
            <ul className="space-y-1">
              {insightDocs.map((doc) => {
