...
export function PostHogHealthMonitor() {
  ...
  const getButtonColor = (): string => {
    if (health.lastError) return 'bg-red-500 hover:bg-red-600';
    if (isNearLimit) return 'bg-yellow-500 hover:bg-yellow-600';
    if (health.enabled) return 'bg-green-500 hover:bg-green-600';
    return 'bg-gray-500 hover:bg-gray-600';
  };

  const getStatusColor = (): string => {
    if (health.lastError) return 'text-red-600';
    if (health.enabled) return 'text-green-600';
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <button
        ...
        className={`rounded-full p-2 shadow-lg transition-colors ${getButtonColor()}`}
