...
export function QuadrantChart({ data, quadrants }: QuadrantChartProps) {
  ...
  const chartData = useMemo(() => {
    return data.map((pr) => {
      ...
      const mainLanguage = pr.commits.reduce((acc, commit) => {
        if (!acc || commit.additions + commit.deletions > acc.changes) {
          return { name: commit.language, changes: commit.additions + commit.deletions };
        }
        return acc;
      }, { name: '', changes: 0 }).name;

      const totalAdditions = pr.commits.reduce((sum, commit) => sum + commit.additions, 0);
      const totalDeletions = pr.commits.reduce((sum, commit) => sum + commit.deletions, 0);
  ...
  return (
    <TooltipProvider>
      <div className="relative w-full aspect-[16/9] bg-background border rounded-lg">
        ...
        {/* Quadrant labels */}
        {Object.entries(QUADRANTS).map(([key, { label, x, y, description }]) => {
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">
                  {quadrantCounts[key.toLowerCase() as keyof typeof quadrantCounts]} files
                </span>
            ...
            <TooltipContent className="max-w-[300px]">
              <div className="space-y-2">
                ...
                <div className="text-xs text-muted-foreground">
                  #{point.pr.number} by {point.pr.author.login} · {format(new Date(point.pr.createdAt), 'MMM d, yyyy')}
                </div>
                <div className="flex gap-4 text-xs">
                  <span className="text-green-500">+{point.stats.additions}</span>
