...
const MockRepositoryHealth = ({
  ...
  const healthData = {
    excellent: {
      ...
      insight:
        'Outstanding repository health! Your team maintains excellent development practices with 98% review coverage and rapid response times.',
      confidence: 92,
    },
    poor: {
      score: 34,
      ...
      insight:
        '⚠️ Critical attention needed: Your repository health is significantly below recommended levels. Immediate action required.',
      confidence: 89,
    },
    warning: {
      score: 67,
      ...
      insight:
        'Your repository health shows room for improvement. Focus on streamlining the review process.',
      confidence: 76,
    },
  };
    ...
    <div className="p-6 border rounded-lg bg-card">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`text-sm font-medium ${data.color}`}>{data.trend}</span>
        </div>
      ...
      {showLLM && (
        <div className={`p-4 ${data.bgColor} rounded-lg`}>
          ...
          <p className={`text-sm ${data.textColor}`}>{data.insight}</p>
        </div>
      )}
      ...
      <div className="mt-4 text-sm text-muted-foreground">
        <p>
          Repository: {owner}/{repo} • Time range: {timeRange}
        </p>
      </div>
    </div>
  );
...
const meta: Meta<typeof MockRepositoryHealth> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'AI-powered repository health assessment component that analyzes multiple health factors and provides intelligent insights about repository maintainability, contributor engagement, and development workflow efficiency.',
      },
    },
  },
  argTypes: {
