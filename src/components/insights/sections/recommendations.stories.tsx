...
const MockRecommendations = ({ owner, repo, timeRange, variant = "default" }: { 
  owner: string; 
  repo: string; 
  timeRange: string;
  ...
  const content = variant === "low-confidence" 
  ...
  const bgColor = variant === "high-priority" ? "bg-red-50 dark:bg-red-950" : "bg-blue-50 dark:bg-blue-950";
  const textColor = variant === "high-priority" ? "text-red-800 dark:text-red-200" : "text-blue-800 dark:text-blue-200";
  const badgeColor = variant === "high-priority" ? "bg-red-200 dark:bg-red-800 text-red-800 dark:text-red-200" : "bg-blue-200 dark:bg-blue-800 text-blue-800 dark:text-blue-200";
    ...
    <div className="p-6 border rounded-lg bg-card">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <h4 className="font-medium">Repository: {owner}/{repo}</h4>
...
const meta: Meta<typeof MockRecommendations> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: "AI-powered recommendations component that provides actionable insights for repository improvement based on health metrics, trends, and activity patterns."
      }
    }
  },
  argTypes: {
...
export const Interactive: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText(/recommendations/i)).toBeInTheDocument();
    
    // Check that AI insight is displayed
    await expect(canvas.getByText(/based on the repository metrics/i)).toBeInTheDocument();
