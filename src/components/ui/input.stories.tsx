...
export const Sizes: Story = {
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: designTokens.spacing[3] }}>
      <div>
        <Label>Small</Label>
        <Input type="text" placeholder="Small input" className="h-8 text-sm" />
      </div>
      <div>
        <Label>Default</Label>
      ...
      <div>
        <Label>Large</Label>
        <Input type="text" placeholder="Large input" className="h-12 text-lg" />
      </div>
    </div>
  ),
  parameters: {
  ...
  render: () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: designTokens.spacing[2] }}>
        ...
        <p className="text-xs text-gray-500">Characters: {value.length}/20</p>
      </div>
    );
  },
  play: ({ canvasElement }) => {
