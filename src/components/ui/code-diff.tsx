      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-md overflow-hidden">
        ...
        <div className="bg-red-50 dark:bg-red-950/20 border-l-4 border-red-500 p-3">
          <div className="flex items-start gap-3">
            ...
            <code className="text-gray-800 dark:text-gray-200 flex-1 break-all">{before}</code>
          </div>
        </div>
        ...
        <div className="bg-green-50 dark:bg-green-950/20 border-l-4 border-green-500 p-3">
          <div className="flex items-start gap-3">
            ...
            <code className="text-gray-800 dark:text-gray-200 flex-1 break-all">{after}</code>
          </div>
        </div>
      </div>
    </div>
...
export const InlineCodeDiff: React.FC<InlineCodeDiffProps> = ({
  ...
  return (
    <div className={cn('inline-flex items-center gap-2 font-mono text-xs', className)}>
      <span className="text-green-600 dark:text-green-400 font-semibold">+{additions}</span>
      ...
      <div className="inline-flex items-center">
        ...
        {[...Array(addedSquares)].map((_, i) => (
          <div key={`add-${i}`} className="bg-green-500 w-2 h-2 mr-0.5" aria-label="Added lines" />
        ))}

        {/* Red boxes for deletions */}
        {[...Array(deletedSquares)].map((_, i) => (
          <div key={`del-${i}`} className="bg-red-500 w-2 h-2 mr-0.5" aria-label="Deleted lines" />
        ))}
...
export const MultiLineDiff: React.FC<MultiLineDiffProps> = ({
  ...
  return (
    <div
      className={cn(
        'font-mono text-sm border border-gray-200 dark:border-gray-700 rounded-md overflow-hidden',
        className
      )}
    >
      {lines.map((line, index) => {
        ...
        return (
          <div
            key={index}
            className={cn('border-l-4 px-3 py-1 flex items-start gap-3', bgColor, borderColor)}
          >
            {showLineNumbers && (
            ...
            <span className={cn('select-none', symbolColor)}>{symbol}</span>
