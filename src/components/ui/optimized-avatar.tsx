...
export interface OptimizedAvatarProps extends Omit<OptimizedAvatarSimpleProps, 'renderAvatar' | 'renderImage' | 'renderFallback'> {}
...
export function OptimizedAvatar(props: OptimizedAvatarProps) {
  ...
  const renderAvatar = React.useCallback((avatarProps: {
  ...
  const renderFallback = React.useCallback((fallbackProps: {
    className: string;
    children: React.ReactNode;
  }) => (
    <AvatarFallback className={fallbackProps.className}>
