...
export function DataStateIndicator({
      ...
      case 'success':
        if (meta_data?.isStale) {
          return {
            ...
            description:
              message ||
              formatLastUpdate(meta_data?.lastUpdate) ||
              'Data from cache • Fresh data loading...',
          };
        }
        return {
  ...
  if (compact) {
    return (
      <div className={cn('inline-flex items-center gap-2 text-sm', indicator.color, className)}>
        {indicator.icon}
  ...
  return (
    <div
      className={cn('rounded-lg border p-4', indicator.bgColor, indicator.borderColor, className)}
    >
      <div className="flex items-start justify-between">
        ...
        {(onRefresh || (metadata?.owner && meta_data?.repo)) &&
          status !== 'pending' &&
        ...
        metadata.dataCompleteness < 75 && (
          <div className="mt-3 pt-3 border-t border-current/10">
            <p className="text-xs text-muted-foreground">
              Want more complete data? The system is gathering additional information in the
              background.
            </p>
          </div>
        )}
